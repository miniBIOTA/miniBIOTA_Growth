---
source: docs/commercial_systems_backlog.md
migrated: 2026-05-05
status: internal-reference
---
# Commercial Systems Backlog Reference

## Use
Use this reference with `skills/commercial-backlog-triage/SKILL.md` when reviewing commercial systems, backlog items, growth infrastructure, or what to build next.

This is planning reference only. It does not create tasks, active projects, scheduled work, approved roadmap items, owners, dates, funding, partner promises, sponsor obligations, sales commitments, public claims, or approved strategy.

## Backlog Items
- Business plan that aligns operations, funding, storytelling, and hardware development.
- Media kit that can support sponsorship outreach, speaking requests, and press conversations.
- Sponsorship package that explains what miniBIOTA offers, what surfaces exist, and how a partner would be represented.
- Speaking and pitching workflow so events are not treated as one-off tasks.
- Newsletter handoff from `email_signups` to an external email platform.

Treat these as backlog needs unless a higher-priority source explicitly approves active work.

## Minimum Pipeline Structure
When a relationship tracker is explicitly authorized, the minimum fields are:
- Opportunity or organization name.
- Opportunity type: sponsorship, speaking, event, donation, affiliate, or other.
- Current status.
- Next action.
- Next action date.
- Owner.
- Estimated value or strategic importance.
- Supporting assets needed.

These fields are a proposed structure, not a created CRM, sales, partner, sponsor, task, or domain-history record.

## Cross-Functional Inputs
- Company Operations: business plan, priority alignment, and decision context.
- Brand & Marketing: sponsor-facing language, founder narrative, and positioning.
- Content Production: proof assets, visuals, and story examples for outreach.
- Web & Digital Presence: donation, events, affiliate, and newsletter capture surfaces.
- Finance & Budget: revenue baseline, overhead picture, liability context, and estimated value assumptions for opportunities.

## Completion Signals
Use these as evaluation criteria only:
- The business plan exists in durable notes rather than as an implied need.
- The media kit and sponsorship package exist as reusable assets.
- At least one working relationship tracker exists in the vault.
- The newsletter capture path has a defined downstream workflow.
- Partnership and sales work can be briefed without reconstructing the function from scratch.

Do not treat a completion signal as complete unless a current source verifies it.

## Notes
- This backlog is about building the operating system for commercial activity, not claiming that the system already exists.
- Once these basics exist, the next step is prioritizing target partner categories and active outreach sequences.
- CFO readiness should stay linked to this backlog so partner and sponsorship design is grounded in real cost pressure and revenue goals.

## Source Notes
- `9. Web & Digital Presence/docs/information_architecture.md`
- `9. Web & Digital Presence/docs/operational_status.md`
- `0. Agent Exports/Executive Brain Build Plan.md`
